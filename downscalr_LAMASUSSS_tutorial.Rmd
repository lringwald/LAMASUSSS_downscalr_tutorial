...
That's it! You successfully downscaled LUC from *Cropland** in Argentina from  projections for the year 2010. The results are stored in `results_DS`. Let's see how the transition **OtherNaturalLand** to **Pasture** from the beginning is allocated now:
...
SRP_prior_sub <- SRP_prior %>% filter(ns %in% unique(start_map_example$ns))
results_DS <- downscalr::downscale(targets = LUC_targets_example,
                                    start.areas =  start_map_example,
                                    xmat = X_long,
                                    betas = beta_hat_mean_long,
